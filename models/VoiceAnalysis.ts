...
const VoiceAnalysisSchema: Schema<IVoiceAnalysis> = new Schema(
  {
    userId: {
...
const RealVoiceAnalysis: Model<IVoiceAnalysis> =
  mongoose.models.VoiceAnalysis || mongoose.model<IVoiceAnalysis>("VoiceAnalysis", VoiceAnalysisSchema);
  ...
  sort(sortOption: any) {
    const executeWithSort = async () => {
      ...
      for (const u of users) {
        const analyses = u.voiceAnalyses || [];
...
const MockVoiceAnalysisModel = {
  ...
  async create(data: any) {
    ...
    if (userIndex >= 0) {
      ...
      if (!user.voiceAnalyses) user.voiceAnalyses = [];
      ...
      user.voiceAnalyses.push(newAnalysis);
...
const VoiceAnalysisProxy = new Proxy(RealVoiceAnalysis, {
  get(target, prop, receiver) {
    if (prop === "find" && global.useMockDatabase) {
      return MockVoiceAnalysisModel.find;
    }
    if (prop === "findOne" && global.useMockDatabase) {
      return MockVoiceAnalysisModel.findOne;
    }
    if (prop === "create" && global.useMockDatabase) {
      return MockVoiceAnalysisModel.create;
    }
    return Reflect.get(target, prop, receiver);
  },
});

export default VoiceAnalysisProxy as unknown as Model<IVoiceAnalysis>;
